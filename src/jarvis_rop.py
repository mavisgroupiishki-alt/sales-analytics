...
def rop_model(calls: List[Dict[str, Any]], analyses: Dict[str, Any]) -> Dict[str, Any]:
    ...
    managers.sort(key=lambda item: (item["critical"], item["attention"], item["review"], item["reanalysis"], -item["coverage"]), reverse=True)

    critical.sort(key=lambda item: str(item["call"].get("created") or ""), reverse=True)
    attention.sort(key=lambda item: (_score(item["analysis"].get("overall_score")) or 10, str(item["call"].get("created") or "")))
...
def render_rop_report(
    ...
    for item in model["attention"][: max(0, 6 - len(model["critical"]))]:
        call, analysis = item["call"], item["analysis"]
        client = (call.get("client") or {}).get("name") or "Клиент не определён"
        manager = (call.get("manager") or {}).get("name") or "Менеджер не определён"
        score = analysis.get("overall_score")
        action = analysis.get("recommended_action") or analysis.get("recommendation") or "Разобрать звонок с менеджером"
        actions += f'''<a class="jr-action low-score" href="/calls/{_text(call.get('activity_id'))}">
          <span class="jr-alert">↓</span><span><b>{_text(client)}</b><small>Низкая оценка {_text(score)}/10 · {_text(manager)}</small>
          <em>{_text(item['reason'])}</em></span><span class="jr-action-task">{_text(action)}</span><span>→</span></a>'''
    if not actions:
        actions = '<div class="jr-empty"><b>Нет звонков, требующих внимания РОПа.</b><span>Нет подтверждённых критичных случаев и оценок 3 или ниже.</span></div>'
    ...
    for item in model["managers"]:
        ...
        manager_rows += f'''<a class="jr-manager-row" href="/managers/{_text(manager.get('id'))}">
          ...
          <span class="jr-risk {'red' if item['critical'] else 'amber' if item['attention'] or item['review'] else 'green'}">{item['critical'] or item['attention'] or item['review'] or '—'}</span></a>'''
    if not manager_rows:
...
_CSS = r'''
.jr-action.low-score .jr-alert{background:var(--amber)}.jr-action.low-score em{color:var(--amber)}
