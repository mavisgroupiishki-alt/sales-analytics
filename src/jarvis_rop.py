...
def filter_calls(
    ...
    if status in {"critical", "needs_review", "normal", "pending"}:
        result = [
            call for call in result
...
def rop_model(calls: List[Dict[str, Any]], analyses: Dict[str, Any]) -> Dict[str, Any]:
    ...
    managers.sort(key=lambda item: (item["critical"], item["review"], -item["coverage"]), reverse=True)
...
<title>Джарвис — отчёт РОПа</title><style>{_CSS}</style></head><body>
...
<main class="jr-shell"><section class="jr-hero"><div><p>ОПЕРАЦИОННЫЙ ОТЧЁТ · {selected_title}</p><h1>Решения, а не<br><span>шум в отчётах.</span></h1><div class="jr-freshness {freshness_class}"><i></i><b>Bitrix24: {freshness_label}</b><span>последняя запись {model['fresh_at']}</span></div></div>
